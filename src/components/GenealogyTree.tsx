...
function buildNode(id: string | undefined, depth: number): TreeNode | { label: string } | undefined {
  if (!id || !id.trim()) return undefined;
  const bird = passaroService.getById(id);
  if (!bird) return { label: id };
  if (depth <= 0) return { bird } as TreeNode;
  return {
    bird,
  ...
  return (
    <div className="flex flex-col items-center gap-1">
      ...
      >
        {isBird ? (
          <>
            <p className="font-heading font-semibold text-foreground leading-tight">{(node as TreeNode).bird.nome}</p>
            <p className="text-muted-foreground" style={{ fontSize: `${scale * 0.75}rem` }}>
              {(node as TreeNode).bird.anilha}
            </p>
          </>
        ) : (
          <p className="text-muted-foreground italic">{(node as { label: string }).label}</p>
        )}
      </div>
